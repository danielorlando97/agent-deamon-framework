 ...
 * **Binary:** `pi` (on `PATH`).
 *
 * **Process:** `pi -p --mode json <prompt>` (prompt as argv).
 *
 * **Stdout:** one JSON object per line; text taken from `text`, `message`, or
 * `output.text` when present.
 *
 * **Stderr:** forwarded as `log` events; tail used on non-zero exit.
 *
 * **Argv bridge:** `-p`, `--mode`, `json`, then `ARGS_JSON` tokens, then the
 * prompt argv.
 */
...
async function emitPiLine(line: string, emit: EmitFn): Promise<void> {
  ...
  const text =
    asStr(o.text) ||
    asStr(o.message) ||
    asStr((o.output as Record<string, unknown>)?.text);
  if (text) await emit({ type: "delta", text: `${text}\n` });
}

export function createPiEngine(timeoutMs: number): EngineDefinition {
  const bin = "pi";
  return {
    info: {
      id: "pi",
      label: "Pi",
      description: "Local `pi -p --mode json` with prompt argument.",
    ...
    async run(ctx) {
      ...
      if (typeof o?.resume === "string") args.push("--resume", o.resume);
      if (o?.model) args.push("--model", o.model);
      args.push(...getEngineArgvExtras(engineId), ctx.message);
      let stderrBuf = "";
      const res = await runLineProcess({
        ...
        timeoutMs,
        signal: ctx.signal,
        onStdoutLine: (line) => emitPiLine(line, ctx.emit),
