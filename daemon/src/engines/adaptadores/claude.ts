/**
 * Engine `claude` — Anthropic Claude Code CLI.
 *
 * **Binary:** `claude` (on `PATH`; availability via `command -v`).
 *
 * **Process:** `claude -p --output-format stream-json --input-format stream-json
 * --verbose --strict-mcp-config --permission-mode bypassPermissions`
 *
 * **Stdin:** stream-json user turn (one JSON line). Built with
 * `buildClaudeStdin` from the chat message string.
 *
 * **Stdout:** one JSON object per line; `type === "assistant"` blocks emit
 * text deltas (and optional thinking logs).
 *
 * **Stderr:** forwarded as `log` events; tail used on non-zero exit.
 *
 * **Argv bridge (`AGENT_ENGINE_*_ARGS_JSON`, see `engines/engine-env.ts`):**
 * default argv, then extra tokens, then stdin payload (no argv prompt).
 */
...
//   --allow-dangerously-skip-permissions              Enable bypassing all permission checks as an option, without it being enabled
...
//   --file <specs...>                                 File resources to download at startup. Format: file_id:relative_path (e.g.,
//                                                     --file file_abc:doc.txt file_def:img.png)
//   --fork-session                                    When resuming, create a new session ID instead of reusing the original (use
//                                                     with --resume or --continue)
//   --from-pr [value]                                 Resume a session linked to a PR by PR number/URL, or open interactive picker
...
//                                                     'sonnet' or 'opus') or a model's full name (e.g. 'claude-sonnet-4-6').
...
//                                                     skipped when Claude is run with the -p mode. Only use this flag in directories
//                                                     you trust.
...
//   --tmux                                            Create a tmux session for the worktree (requires --worktree). Uses iTerm2
...
//                                                     all tools, "default" to use all tools, or specify tool names (e.g.
//                                                     "Bash,Edit,Read").
...
//   doctor                                            Check the health of your Claude Code auto-updater. Note: The workspace trust
...
import {
  buildClaudeStdin,
  emitClaudeStreamJsonLine,
} from "./lib/claude-stream-json.js";
...
export function createClaudeEngine(timeoutMs: number): EngineDefinition {
  const bin = "claude";
  return {
    info: {
      id: "claude",
    ...
    async run(ctx) {
      ...
      const args = [
        ...
        "--strict-mcp-config",
        "--permission-mode",
        o?.permissionMode?.trim() || "bypassPermissions",
      ];
      if (o?.continueSession) args.push("-c");
      if (typeof o?.resume === "string") args.push("-r", o.resume);
      if (o?.resume === true) args.push("-c");
      ...
      const stdin = buildClaudeStdin(ctx.message);
      let stderrBuf = "";
      const res = await runLineProcess({
        ...
        timeoutMs,
        signal: ctx.signal,
        onStdoutLine: (line) => emitClaudeStreamJsonLine(line, ctx.emit),
